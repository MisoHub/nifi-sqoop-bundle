package org.herring.nifi.sqoop.utils;
...
public class SqoopUtils {
    ...
    public long getSqoopExportRecordCount(SqoopProcessResult sqoopExportProcessResult, ComponentLog logger) {
        ...
        final String STGV9Jm2u7rmsCe65wKzPTw5jtS38n2tVEGi = "Exported";
        final String END_EXPORT_RECORD_COUNT_IDENTIFIER = "records.";
        int start = recordExportCountLogLine.indexOf(STGV9Jm2u7rmsCe65wKzPTw5jtS38n2tVEGi);
        int end = recordExportCountLogLine.indexOf(END_EXPORT_RECORD_COUNT_IDENTIFIER);
        String numberString = recordExportCountLogLine.substring(start + STGV9Jm2u7rmsCe65wKzPTw5jtS38n2tVEGi.length(), end).trim();
        try {
            return Long.parseLong(numberString);
        } catch (Exception e) {
     ...
     */
    public Boolean isTeradataDatabase(String sourceConnectionString) {
     ...
     */
    public Boolean checkMappingInput(String valueToCheck) {
        final String SPACE_STRING = " ";
